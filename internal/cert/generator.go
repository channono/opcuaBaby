...
func GenerateCSRFiles(config *CertificateConfig, csrPath, keyPath string) (string, string, error) {
	...
	if csrPath == "" {
		csrPath = filepath.Join(os.TempDir(), "opcuababy", "client.csr")
	}
	if keyPath == "" {
		keyPath = filepath.Join(os.TempDir(), "opcuababy", "client.key")
	}
...
func GenerateSelfSignedCertificateFiles(config *CertificateConfig, certPath, keyPath string) error {
	...
	if certPath != "" {
		if err := copyFile(out.CertDERPath, certPath); err != nil {
			return err
		}
	}
	if keyPath != "" {
		if err := copyFile(out.KeyPKCS1Path, keyPath); err != nil {
			return err
		}
	}
	return nil
...
func generateSelfSignedClientCert(config *CertificateConfig, dir string) (*genOutput, error) {
	...
	if len(config.DNSNames) > 0 {
		tmpl.DNSNames = append(tmpl.DNSNames, config.DNSNames...)
	}
	if len(config.IPAddresses) > 0 {
		tmpl.IPAddresses = append(tmpl.IPAddresses, config.IPAddresses...)
	}
	...
	if err := os.WriteFile(out.CertDERPath, certDER, 0644); err != nil {
		return nil, fmt.Errorf("write selfsigned.der: %w", err)
	}
	if err := os.WriteFile(out.CertPEMPath, pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: certDER}), 0644); err != nil {
		return nil, fmt.Errorf("write selfsigned.crt: %w", err)
	}
	if err := os.WriteFile(out.KeyPKCS1Path, pem.EncodeToMemory(&pem.Block{Type: "RSA PRIVATE KEY", Bytes: x509.MarshalPKCS1PrivateKey(key)}), 0600); err != nil {
		return nil, fmt.Errorf("write selfsigned.key: %w", err)
	}
	if pkcs8, err := x509.MarshalPKCS8PrivateKey(key); err == nil {
		if err := os.WriteFile(out.KeyPKCS8Path, pem.EncodeToMemory(&pem.Block{Type: "PRIVATE KEY", Bytes: pkcs8}), 0600); err != nil {
			return nil, fmt.Errorf("write selfsigned.pem: %w", err)
		}
	} else {
...
func generateClientCertSignedByLocalCA(config *CertificateConfig, dir string) (*genOutput, error) {
	...
	if caBlk == nil || caBlk.Type != "CERTIFICATE" {
		return nil, fmt.Errorf("invalid ca.crt")
	}
	...
	if caKeyBlk == nil || caKeyBlk.Type != "RSA PRIVATE KEY" {
	...
	// client.der (DER cert)
	if err := os.WriteFile(out.CertDERPath, certDER, 0644); err != nil {
		return nil, fmt.Errorf("write client.der: %w", err)
	}
	// client.crt (PEM cert)
	if err := os.WriteFile(out.CertPEMPath, pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: certDER}), 0644); err != nil {
		return nil, fmt.Errorf("write client.crt: %w", err)
	}
	...
	if pkcs8, err := x509.MarshalPKCS8PrivateKey(key); err == nil {
		if err := os.WriteFile(out.KeyPKCS8Path, pem.EncodeToMemory(&pem.Block{Type: "PRIVATE KEY", Bytes: pkcs8}), 0600); err != nil {
			return nil, fmt.Errorf("write client.pem: %w", err)
		}
	} else {
		return nil, fmt.Errorf("marshal PKCS#8: %w", err)
	}
