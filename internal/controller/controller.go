...
func (c *Controller) Connect(cfg *opc.Config) error {
	...
	// Build endpoint candidates and honor requested AuthMode. Try Anonymous across endpoints when selected.
	var opts []opcua.Option
	connectURL := cfg.EndpointURL
	if eps, err := opcua.GetEndpoints(ctx, cfg.EndpointURL); err == nil {
		// Helper to inspect user token support and policyID
		getPolicySupport := func(ep *ua.EndpointDescription) (userPID string, supportsUser, supportsAnon bool) {
			if ep == nil {
				return "", false, false
			}
		...
		// Collect endpoints we may try according to cfg.SecurityPolicy/Mode preference (when specified)
		type endpointRef struct{ ep *ua.EndpointDescription }
		all := make([]endpointRef, 0, len(eps))
		for _, ep := range eps {
			if ep == nil {
				continue
			}
			all = append(all, endpointRef{ep: ep})
		}
		// Optional filter by cfg.SecurityPolicy/Mode if not Auto/empty
		wantPol := strings.ToLower(strings.TrimSpace(cfg.SecurityPolicy))
		wantMode := strings.ToLower(strings.TrimSpace(cfg.SecurityMode))
		filtered := all
		if (wantPol != "" && wantPol != "auto") || (wantMode != "" && wantMode != "auto") {
			tmp := make([]endpointRef, 0, len(all))
			for _, r := range all {
				pol := strings.ToLower(r.ep.SecurityPolicyURI)
				mode := strings.ToLower(r.ep.SecurityMode.String())
				matchPol := wantPol == "" || wantPol == "auto" || strings.Contains(pol, wantPol)
				matchMode := wantMode == "" || wantMode == "auto" || strings.EqualFold(mode, wantMode)
		...
		// Helper to prepare secure channel credentials once
		type keymat struct {
			key      *rsa.PrivateKey
			cert     []byte
			ident    []opcua.Option
		...
		prepareKeymat := func() *keymat {
			km := &keymat{}
			// Read and parse key/cert only once
			...
			if keyBytes, err := os.ReadFile(cfg.KeyFile); err == nil {
				...
				if b, _ := pem.Decode(keyBytes); b != nil {
					if x509.IsEncryptedPEMBlock(b) || len(b.Headers) > 0 {
						c.Log("[red]Encrypted private key not supported; not attaching client certificate[-]")
						return nil
					}
					keyDER = b.Bytes
				} else {
				...
				if k1, e1 := x509.ParsePKCS1PrivateKey(keyDER); e1 == nil {
					...
					if rk, ok := kIf.(*rsa.PrivateKey); ok {
						km.key = rk
					} else {
						c.Log("[red]Private key is not RSA; not attaching client certificate[-]")
						return nil
					}
				} else {
					c.Log("[red]Failed to parse private key; not attaching client certificate[-]")
					return nil
				}
			} else {
				c.Log("[red]Failed to read key file; not attaching client certificate[-]")
				return nil
			}
			...
			if certBytes, err := os.ReadFile(cfg.CertFile); err == nil {
				if strings.Contains(string(certBytes), "-----BEGIN") {
					...
					if km.cert == nil {
						c.Log("[red]No CERTIFICATE block found in PEM; not attaching client certificate[-]")
						return nil
					}
				} else {
					if _, e := x509.ParseCertificate(certBytes); e == nil {
						...
						c.Log("[red]Failed to parse certificate (DER); not attaching client certificate[-]")
						return nil
					}
				}
				if pc, e := x509.ParseCertificate(km.cert); e == nil {
					fp := sha1.Sum(km.cert)
					c.Log(fmt.Sprintf("[cyan]Client cert SHA1: %s[-]", strings.ToUpper(hex.EncodeToString(fp[:]))))
					if len(pc.URIs) > 0 && pc.URIs[0] != nil {
						km.appURI = pc.URIs[0].String()
						km.ident = append(km.ident, opcua.ApplicationURI(km.appURI))
					}
					if len(pc.URIs) > 0 {
						for _, u := range pc.URIs {
							if u != nil {
								km.certURIs = append(km.certURIs, u.String())
							}
						}
					}
				}
			} else {
				c.Log("[red]Failed to read certificate file; not attaching client certificate[-]")
				return nil
			}
			return km
		}

		// Decide order based on cfg.AuthMode: Anonymous, Username, or Auto (Anonymous first)
		tryAnonymous := func() bool { return !strings.EqualFold(strings.TrimSpace(cfg.AuthMode), "username") }
		tryUsername := func() bool {
			return !strings.EqualFold(strings.TrimSpace(cfg.AuthMode), "anonymous") && cfg.Username != ""
		}

		// Attempt Anonymous on any endpoint that supports it
		if tryAnonymous() {
			var lastErr error
			attempted := 0
			var km *keymat
			for _, r := range filtered {
				_, _, hasAnon := getPolicySupport(r.ep)
				if !hasAnon {
					continue
				}
				// Build options
				optsAnon := []opcua.Option{
					opcua.SecurityFromEndpoint(r.ep, ua.UserTokenTypeAnonymous),
					opcua.AuthAnonymous(),
					opcua.ApplicationName("opcuababy"),
				}
				// For secure endpoints, only attempt if a client cert/key is configured
				if r.ep.SecurityMode != ua.MessageSecurityModeNone {
					if km == nil {
						km = prepareKeymat()
					}
					if km == nil {
						c.Log("[yellow]Skip Anonymous secure endpoint (Sign/SignAndEncrypt): client certificate/key not configured[-]")
						continue
					}
					optsAnon = append(optsAnon, opcua.PrivateKey(km.key), opcua.Certificate(km.cert))
					if len(km.ident) > 0 {
						optsAnon = append(optsAnon, km.ident...)
					}
				}
				attempted++
				...
				if err := tmpCli.Connect(ctx); err != nil {
					lastErr = err
					c.Log(fmt.Sprintf("[red]Connect failed (Anonymous %s/%s): %v[-]", r.ep.SecurityPolicyURI, r.ep.SecurityMode.String(), err))
				...
				c.mu.Unlock()
				tmpCli.Handler = c
				c.Log(fmt.Sprintf("[green]Connected to %s (Anonymous, %s/%s)[-]", cfg.EndpointURL, r.ep.SecurityPolicyURI, r.ep.SecurityMode.String()))
				if c.OnConnectionStateChange != nil {
					c.OnConnectionStateChange(true, cfg.EndpointURL, nil)
				}
				return nil
			}
			if attempted > 0 && !tryUsername() {
				// Do not auto-fallback to Username if user requested Anonymous explicitly
				c.mu.Lock()
				c.isConnecting = false
				c.mu.Unlock()
				if lastErr == nil {
					lastErr = fmt.Errorf("all Anonymous candidates failed")
				}
				if c.OnConnectionStateChange != nil {
		...
		// Attempt Username if configured
		if tryUsername() {
			...
			for _, r := range filtered {
				pid, hasUser, _ := getPolicySupport(r.ep)
				if hasUser {
					cands = append(cands, candidate{ep: r.ep, pid: pid})
				}
			}
			// Prepare keymat if any secure endpoint is present and cert/key configured
			needSecure := false
			...
			if needSecure && km == nil {
				c.Log("[yellow]Secure endpoints (Sign/SignAndEncrypt) will be skipped: client certificate/key not configured[-]")
			}
			...
			for _, cand := range cands {
				tryOpts := []opcua.Option{
					opcua.SecurityFromEndpoint(cand.ep, ua.UserTokenTypeUserName),
					opcua.AuthUsername(cfg.Username, cfg.Password),
					opcua.ApplicationName("opcuababy"),
				}
				if cand.pid != "" {
					tryOpts = append(tryOpts, opcua.AuthPolicyID(cand.pid))
				}
				if cand.ep.SecurityMode != ua.MessageSecurityModeNone {
					if km == nil {
						c.Log("[yellow]Skip Username secure endpoint (Sign/SignAndEncrypt): client certificate/key not configured[-]")
						continue
					}
					tryOpts = append(tryOpts, opcua.PrivateKey(km.key), opcua.Certificate(km.cert))
					if len(km.ident) > 0 {
						tryOpts = append(tryOpts, km.ident...)
					}
				}
				attempted++
			...
			if attempted > 0 {
				...
				c.mu.Unlock()
				if lastErr == nil {
					lastErr = fmt.Errorf("all Username candidates failed")
				}
				if c.OnConnectionStateChange != nil {
					c.OnConnectionStateChange(false, cfg.EndpointURL, lastErr)
				}
	...
	// Fallback if discovery failed or no endpoints matched filters: use None/None Anonymous
	if len(opts) == 0 {
		opts = []opcua.Option{
			...
			if strings.Contains(lower, "typemismatch") || strings.Contains(lower, "bad_type") {
				...
				if reflect.ValueOf(writeValue).Kind() != reflect.Slice {
					...
					if s != "" {
						...
						switch dt {
						case "float", "float32":
							v, perr := strconv.ParseFloat(items[0], 32)
							buildErr = perr
							if buildErr == nil {
								arr = []float32{float32(v)}
							}
						case "double", "float64":
							v, perr := strconv.ParseFloat(items[0], 64)
							buildErr = perr
							if buildErr == nil {
								arr = []float64{v}
							}
						case "int16":
							v, perr := strconv.ParseInt(items[0], 10, 16)
							buildErr = perr
							if buildErr == nil {
								arr = []int16{int16(v)}
							}
						case "uint16":
							v, perr := strconv.ParseUint(items[0], 10, 16)
							buildErr = perr
							if buildErr == nil {
								arr = []uint16{uint16(v)}
							}
						case "int32":
							v, perr := strconv.ParseInt(items[0], 10, 32)
							buildErr = perr
							if buildErr == nil {
								arr = []int32{int32(v)}
							}
						case "uint32":
							v, perr := strconv.ParseUint(items[0], 10, 32)
							buildErr = perr
							if buildErr == nil {
								arr = []uint32{uint32(v)}
							}
						case "int64":
							v, perr := strconv.ParseInt(items[0], 10, 64)
							buildErr = perr
							if buildErr == nil {
								arr = []int64{v}
							}
						case "uint64":
							v, perr := strconv.ParseUint(items[0], 10, 64)
							buildErr = perr
							if buildErr == nil {
								arr = []uint64{v}
							}
						case "boolean", "bool":
							v, perr := strconv.ParseBool(items[0])
							buildErr = perr
							if buildErr == nil {
								arr = []bool{v}
							}
						...
						case "datetime":
							t, perr := time.Parse("2006-01-02 15:04:05.999999999", items[0])
							buildErr = perr
							if buildErr == nil {
								arr = []time.Time{t}
							}
