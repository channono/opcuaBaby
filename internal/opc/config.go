...
func (c *Config) ToOpcuaOptions() ([]opcua.Option, error) {
	...
	// Normalize policy: allow short names or full URIs
	switch strings.ToLower(pol) {
	case "", "auto":
		// if unspecified and mode is none, use None; otherwise require explicit policy
	...
	case "none":
		pol = "None"
	case "basic128rsa15", "basic256", "basic256sha256":
		// leave as short form; opcua.SecurityPolicy accepts short names
	default:
		// accept full URI (http...)
	...
	// If secure channel requires signing or encryption, ensure application certificate/private key are provided
	requiresSecureChannelCert := !strings.EqualFold(c.SecurityMode, "None") &&
		(strings.EqualFold(c.SecurityMode, "Sign") || strings.EqualFold(c.SecurityMode, "SignAndEncrypt"))
	if requiresSecureChannelCert {
		if c.CertFile == "" || c.KeyFile == "" {
			return nil, fmt.Errorf("client certificate and private key are required for SecurityMode %s. Please generate certificates first using the UI", c.SecurityMode)
		}
